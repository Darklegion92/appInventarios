...
function FacturasVentas() {
  ...
  const grabarFactura = async () => {
    if (cambio >= 0) {
      if (totalFactura > 0) {
        ...
        setTituloConformacion("GUARDAR ESTA ENTRADA");
        ...
        setMsgAlerta("La entrada no puede estar en 0");
        setModalAlerta(true);
      }
    } 
  };
  ...
  const handleOkConfirmacion = async () => {
    ...
    if (resp.res) {
      setTituloAlerta("ENTRADA DE INVENTARIO");
      setIconoAlerta(
        <CheckCircleOutlined
          style={{
            fontSize: "70px",
      ...
      setMsgAlerta("Entrada de Inventario Guardada Correctamente");
      setModalAlerta(true);
