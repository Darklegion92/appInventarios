...
function FacturasCompras() {
  ...
  const grabarFactura = async () => {
    if (totalFactura > 0) {
      ...
      setTituloConformacion("GUARDAR FACTURA DE COMPRA");
  ...
  const handleOkConfirmacion = async () => {
    ...
    if (resp.res) {
      setTituloAlerta("FACTURA DE COMPRA");
      setIconoAlerta(
        <CheckCircleOutlined
          style={{
            fontSize: "70px",
      ...
      setMsgAlerta("Factura De Compra Guardada Correctamente");
      setModalAlerta(true);
  ...
  const onDoubleClick = (e) => {
    ...
    const valor = parseFloat(totalFactura) - parseFloat(elim[0].valorTotal) || 0;
    setTotalIVA(iva);
    settotalFactura(valor);
  };

  const cargarDatos = async () => {
    const datos = await axios.get(API + "parametros/facturascompra/numero", {
      headers: {
        authorization: sessionStorage.getItem("Token"),
      },
    });
